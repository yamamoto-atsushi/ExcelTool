...
class ExcelSearchTool:
                    ...
                    try:
                        ...
                        for keyword in self.keywords:
                            if keyword in matched_keywords:
                                continue  # 既に見つかっているキーワードはスキップ
                                
                            search_keyword = keyword if self.case_sensitive else keyword.lower()
                            
                            # データフレーム全体を文字列に変換して検索
                            df_str = df.astype(str)
                            found = False
                            
                            for col in df_str.columns:
                                for val in df_str[col]:
                                    val_str = str(val) if self.case_sensitive else str(val).lower()
                                    if search_keyword in val_str and val_str != 'nan':
    ...
    def save_results(self):
        ...
        try:
            ...
            print(f"[INFO] 合計 {len(self.results)} 件のファイルでキーワードが見つかりました")
